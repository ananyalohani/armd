...
import { writerDb, readerDb } from "./services/prisma";
...
app.post("/sessions", async (req, res) => {
  ...
  try {
    ...
    events.map(async (e) => {
      await writerDb.sessionEvent.create({
        data: {
          type: e.type,
...
app.get("/persons", async (req, res) => {
  const persons = await readerDb.person.findMany();
  res.json(persons);
});

app.get("/persons/:id", async (req, res) => {
  const person = await readerDb.person.findUnique({
    where: {
      id: req.params.id,
    },
  });
